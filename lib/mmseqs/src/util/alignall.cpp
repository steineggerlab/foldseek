...
int alignall(int argc, const char **argv, const Command &command) {
    ...
    DBReader<DBKeyType> tdbr(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<DBKeyType>::USE_DATA|DBReader<DBKeyType>::USE_INDEX);
    ...
    DBReader<DBKeyType> dbr_res(par.db2.c_str(), par.db2Index.c_str(), par.threads, DBReader<DBKeyType>::USE_DATA|DBReader<DBKeyType>::USE_INDEX);
...
#pragma omp for schedule(dynamic, 1)
            for (size_t id = start; id < (start + bucketSize); id++) {
                ...
                const DBKeyType key = dbr_res.getDbKey(id);
