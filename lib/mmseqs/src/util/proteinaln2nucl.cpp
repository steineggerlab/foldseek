...
int proteinaln2nucl(int argc, const char **argv, const Command &command) {
    ...
    DBReader<DBKeyType> qdbr_nuc(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX | DBReader<DBKeyType>::USE_DATA);
    qdbr_nuc.open(DBReader<DBKeyType>::NOSORT);
    qdbr_nuc.readMmapedDataInMemory();

    DBReader<DBKeyType> qdbr_aa(par.db3.c_str(), par.db3Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX | DBReader<DBKeyType>::USE_DATA);
    qdbr_aa.open(DBReader<DBKeyType>::NOSORT);
    qdbr_aa.readMmapedDataInMemory();

    DBReader<DBKeyType> *tdbr_nuc = NULL;
    DBReader<DBKeyType> *tdbr_aa = NULL;
//    NucleotideMatrix subMat(par.scoringMatrixFile.values.nucleotide().c_str(), 1.0, 0.0);
    ...
    if (par.db1.compare(par.db2) == 0 && par.db3.compare(par.db4) == 0) {
        ...
        tdbr_nuc = new DBReader<DBKeyType>(par.db2.c_str(), par.db2Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX | DBReader<DBKeyType>::USE_DATA);
        tdbr_nuc->open(DBReader<DBKeyType>::NOSORT);
        tdbr_nuc->readMmapedDataInMemory();

        tdbr_aa = new DBReader<DBKeyType>(par.db4.c_str(), par.db4Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX | DBReader<DBKeyType>::USE_DATA);
        tdbr_aa->open(DBReader<DBKeyType>::NOSORT);
