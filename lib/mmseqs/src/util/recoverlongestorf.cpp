...
int recoverlongestorf(int argc, const char **argv, const Command &command) {
    ...
    std::unordered_map<DBKeyType, std::pair<DBKeyType, size_t>> contigToLongest;
#pragma omp parallel
    {
...
#endif 
        std::unordered_map<DBKeyType, std::pair<DBKeyType, size_t>> localContigToLongest;
...
#pragma omp for schedule(dynamic, 100)
        for (size_t id = 0; id < headerReader.getSize(); ++id) {
            ...
            DBKeyType contigKey = orf.id;
            size_t orfLen = std::max(orf.from, orf.to) - std::min(orf.from, orf.to) + 1;
            std::unordered_map<DBKeyType, std::pair<DBKeyType, size_t>>::iterator it = localContigToLongest.find(contigKey);
            if (it != localContigToLongest.end()) {
                std::pair<DBKeyType, size_t> orfKeyToLength = it->second;
...
#pragma omp critical
    ...
    std::unordered_set<DBKeyType> acceptedContigs;
    std::unordered_set<DBKeyType> eliminatedContigs;
#pragma omp parallel
    {
        int thread_idx = 0;
#ifdef OPENMP
        ...
        std::unordered_set<DBKeyType> localAcceptedContigs;
        std::unordered_set<DBKeyType> localEliminatedContigs;
...
#pragma omp critical
    ...
    for (auto contigIt = eliminatedContigs.begin(); contigIt != eliminatedContigs.end(); ++contigIt) {
        DBKeyType contigKey = *contigIt;
        std::unordered_map<DBKeyType, std::pair<DBKeyType, size_t>>::iterator it = contigToLongest.find(contigKey);
        if (it != contigToLongest.end()) {
            DBKeyType orfKey = it->second.first;
