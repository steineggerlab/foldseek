...
int doExtract(Parameters &par, DBReader<DBKeyType> &blastTabReader,
    ...
    if (par.msaType == 0) {
        ...
        headerReader = new DBReader<DBKeyType>(msaHeaderDataName.c_str(), msaHeaderIndexName.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
        headerReader->open(DBReader<DBKeyType>::SORT_BY_LINE);

        sequenceReader = new DBReader<DBKeyType>(msaSequenceDataName.c_str(), msaSequenceIndexName.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
        sequenceReader->open(DBReader<DBKeyType>::SORT_BY_LINE);
    }

    DBReader<DBKeyType> msaReader(msaDataName.c_str(), msaIndexName.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
    msaReader.open(DBReader<DBKeyType>::NOSORT);
