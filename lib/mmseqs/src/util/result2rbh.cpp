...
int result2rbh(int argc, const char **argv, const Command &command) {
    ...
    DBReader<DBKeyType> resultReader(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
...
#pragma omp for schedule(dynamic, 1)
        for (size_t id = 0; id < resultReader.getSize(); id++) {
            ...
            DBKeyType AdbID = resultReader.getDbKey(id);
