...
#define X(g,r) \
    template void call_amino_gpu_localAlignmentKernel_affinegap_floatOrInt_pssm_singletile<float, 512, g, r, withEndPosition, subjectIsCaseSensitive, ScoreOutputIterator>( \
        int, \
        const char * const, \
...
#define subjectIsCaseSensitive true
#define X(g,r) \
    template void call_amino_gpu_localAlignmentKernel_affinegap_floatOrInt_pssm_multitile<float, 512, g, r, withEndPosition, subjectIsCaseSensitive, ScoreOutputIterator>( \
        int, \
        const char * const, \
        ScoreOutputIterator const, \
        const size_t* const, \
