...
namespace hardcodedzero{
    ...
    #define X(g,r) \
        template void call_GaplessFilter_strided_PSSM_singletile_kernel<half2, 512, g, r, subjectIsCaseSensitive, ScoreOutputIterator>( \
            const char * const, \
    ...
    #define subjectIsCaseSensitive true
    #define X(g,r) \
        template void call_GaplessFilter_strided_PSSM_multitile_kernel<half2, 512, g, r, subjectIsCaseSensitive, ScoreOutputIterator>( \
            int, \
            const char * const, \
            ScoreOutputIterator const, \
            const size_t* const, \
