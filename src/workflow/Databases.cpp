...
extern bool hide_base_downloads;
extern std::vector<DatabaseDownload> externalDownloads;
...
std::string listDatabases(const Command &command, std::vector<DatabaseDownload> &downloads, bool detailed) {
...
int databases(int argc, const char **argv, const Command &command) {
    ...
    std::vector<DatabaseDownload> usedDownloads = downloads;
    if(hide_base_downloads){
        usedDownloads=externalDownloads;
    }
    ...
    for (size_t i = 0; i < usedDownloads.size(); ++i) {
        if (par.db1 == std::string(usedDownloads[i].name)) {
            downloadIdx = i;
            break;
        }
    }
