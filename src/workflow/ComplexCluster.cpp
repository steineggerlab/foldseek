...
int complexcluster(int argc, const char **argv, const Command &command) {
    ...
    std::string hash = SSTR(par.hashParameter(command.databases, par.filenames, par.complexclusterworkflow));
    if (par.reuseLatest) {
    ...
    // REVIEW : the variables below are declared in the complexcluster.sh
