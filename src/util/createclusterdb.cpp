...
int createclusearchdb(int argc, const char **argv, const Command& command) {
    ...
    DBReader<DBKeyType> clusterReader(par.db2.c_str(), par.db2Index.c_str(), par.threads,
                                         DBReader<DBKeyType>::USE_DATA | DBReader<DBKeyType>::USE_INDEX);
    clusterReader.open(DBReader<DBKeyType>::NOSORT);
    ...
    for(size_t prefix = 0; prefix < suffixes.size(); prefix++) {
        ...
        DBReader<DBKeyType> reader(db1.c_str(), db1Index.c_str(), par.threads,
                                  DBReader<DBKeyType>::USE_DATA | DBReader<DBKeyType>::USE_INDEX);
    ...
    #pragma omp for schedule(static)
        ...
        // merge index
        DBReader<DBKeyType> dbrRep(repDbSeq.c_str(), repDbSeqIdx.c_str(), par.threads,
                                      DBReader<DBKeyType>::USE_INDEX);
        dbrRep.open(DBReader<DBKeyType>::NOSORT);
        DBReader<DBKeyType> dbrSeq(seqsDbSeq.c_str(), seqsDbSeqIdx.c_str(), par.threads,
                                      DBReader<DBKeyType>::USE_INDEX);
        dbrSeq.open(DBReader<DBKeyType>::NOSORT);
        std::string seqsDbSeqIdxTmp = seqsDbSeqIdx + "_tmp";

        FILE *sIndex = FileUtil::openAndDelete(seqsDbSeqIdxTmp.c_str(), "w");
        std::vector<DBReader<DBKeyType>::Index> allIndex(dbrSeq.getSize() + dbrRep.getSize());
        size_t dataSize = 0;
        ...
        for (size_t i = 0; i < dbrSeq.getSize(); i++) {
            DBReader<DBKeyType>::Index *index = dbrSeq.getIndex(i);
            index->offset += dataSize;
            allIndex[dbrRep.getSize() + i] = *index;
        }
        SORT_PARALLEL(allIndex.begin(), allIndex.end(), DBReader<DBKeyType>::Index::compareById);
        char buffer[1024];
    ...
    clusterReader.close();
    DBReader<DBKeyType>::copyDb(par.db2, par.db3 + "_clu");
    ...
    for (size_t i = 0; i < ARRAY_SIZE(suffices); ++i) {
        ...
        if (suffices[i].flag && FileUtil::fileExists(file.c_str())) {
            DBReader<DBKeyType>::copyDb(file, par.db3 + suffices[i].suffix);
        }
    }
    for (size_t i = 0; i < ARRAY_SIZE(suffices); ++i) {
        ...
        if (suffices[i].flag && FileUtil::fileExists(file.c_str())) {
            ...
            if (FileUtil::fileExists(fileToLinkTo.c_str())){
                DBReader<DBKeyType>::removeDb(fileToLinkTo);
            }
            DBReader<DBKeyType>::aliasDb(file, fileToLinkTo);
        }
    }
    return EXIT_SUCCESS;
}
