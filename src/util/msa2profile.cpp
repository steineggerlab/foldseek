...
int msa2profile(int argc, const char **argv, const Command &command) {
    ...
    DBReader<DBKeyType> qDbr(msaData.c_str(), msaIndex.c_str(), par.threads, mode);
...
#pragma omp for schedule(dynamic, 1)
        for (size_t id = 0; id < qDbr.getSize(); ++id) {
            ...
            while (kseq_read(seq) >= 0) {
                ...
                if (setSize == 0) {
                    ...
                    if ((mode & DBReader<DBKeyType>::USE_LOOKUP) == 0) {
            ...
            if (mode & DBReader<DBKeyType>::USE_LOOKUP) {
    ...
    DBReader<DBKeyType>::copyDb(par.db1, par.db2, (DBFiles::Files)(DBFiles::LOOKUP | DBFiles::SOURCE));
