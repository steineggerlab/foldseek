...
int orftocontig(int argn, const char **argv, const Command& command) {
    ...
    DBReader<DBKeyType> contigsReader(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
    contigsReader.open(DBReader<DBKeyType>::NOSORT);

    // info will be obtained from orf headers:
    DBReader<DBKeyType> orfHeadersReader(par.hdr2.c_str(), par.hdr2Index.c_str(), par.threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
    orfHeadersReader.open(DBReader<DBKeyType>::LINEAR_ACCCESS);
...
#pragma omp for schedule(dynamic, 100)
        for (size_t id = 0; id < orfHeadersReader.getSize(); ++id) {
            ...
            DBKeyType orfKey = orfHeadersReader.getDbKey(id);
