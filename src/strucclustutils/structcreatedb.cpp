...
#pragma omp parallel default(none) shared(par, torsiondbw, hdbw, cadbw, aadbw, idbw, mat, looseFiles, progress, globalCnt, globalFileidCnt, entrynameToFileId, filenameToFileId, fileIdToName, mappingWriter, foldcompWriter, inputFormat, inputCompressionFormat) reduction(+:incorrectFiles, tooShort, notProtein, needToWriteModel)
    {
        unsigned int thread_idx = 0;
#ifdef OPENMP
        thread_idx = static_cast<unsigned int>(omp_get_thread_num());
...
#pragma omp parallel default(none) shared(par, torsiondbw, hdbw, cadbw, aadbw, idbw, mat, progress, globalCnt, globalFileidCnt, entrynameToFileId, filenameToFileId, fileIdToName, reader, mappingWriter, foldcompWriter, inputFormat, inputCompressionFormat) reduction(+:incorrectFiles, tooShort, notProtein, needToWriteModel)
        {
            StructureTo3Di structureTo3Di;
            PulchraWrapper pulchra;
            GemmiWrapper readStructure;
