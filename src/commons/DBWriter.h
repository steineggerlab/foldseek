...
public:
    ...
    void writeEnd(DBKeyType key, unsigned int thrIdx = 0, bool addNullByte = true, bool addIndexEntry = true);

    void writeData(const char *data, size_t dataSize, DBKeyType key, unsigned int threadIdx = 0, bool addNullByte = true, bool addIndexEntry = true);

    static size_t indexToBuffer(char *buff1, DBKeyType key, size_t offsetStart, size_t len);

    void alignToPageSize(int thrIdx = 0);

    void sortDatafileByIdOrder(DBReader<DBKeyType>& qdbr);
    ...
    static void createRenumberedDB(const std::string& dataFile, const std::string& indexFile, const std::string& origData, const std::string& origIndex, int sortMode = DBReader<DBKeyType>::SORT_BY_ID_OFFSET);
